...
from Config.config import mysql_host, mysql_base, mysql_table, mysql_passwd, redis_passwd, redis_host, max_EmailCodeTime
...
redis_pool = redis.ConnectionPool(
    host=redis_host, port=6379, db=0, password=redis_passwd, decode_responses=True)
...
def query_redis(username: str):
    ...
    # 通过docker部署需要使用下面的配置连接
    db = pymysql.Connect(host=mysql_host, user='root', password=mysql_passwd,
                         database=mysql_base, charset='utf8')
